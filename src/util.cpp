...
int nAnonymizeDemocraticoinAmount = 1000;
...
#else
    const char* pszModule = "Democraticoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Democraticoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Democraticoin
    // Mac: ~/Library/Application Support/Democraticoin
    // Unix: ~/.Democraticoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Democraticoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Democraticoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Democraticoind.pid"));
