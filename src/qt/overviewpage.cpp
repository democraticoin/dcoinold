...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDemocraticoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDemocraticoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDemocraticoinAmount = strAnonymizeDemocraticoinAmount.remove(strAnonymizeDemocraticoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDemocraticoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDemocraticoinAmount*COIN) nMaxToAnonymize = nAnonymizeDemocraticoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDemocraticoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDemocraticoinAmount));
        strAnonymizeDemocraticoinAmount = strAnonymizeDemocraticoinAmount.remove(strAnonymizeDemocraticoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDemocraticoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDemocraticoinAmount == 0){
