...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Democraticoin"))
        return false;
...
{
    // Convert Democraticoin:// to Democraticoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Democraticoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Democraticoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Democraticoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Democraticoin.lnk
...
{
    return GetAutostartDir() / "Democraticoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Democraticoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Democraticoin";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Democraticoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Democraticoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Democraticoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Democraticoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Democraticoin-Qt"));
