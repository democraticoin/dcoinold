...
// Copyright (c) 2009-2018 The Ferrum developers
...
const string strMessageMagic = "Democraticoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (fRejectInsaneFee && nFees > MIN_RELAY_TX_FEE * 1000000)
            return error("AcceptableInputs: : insane fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 1000000);
                         ...
                         bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        if (fRejectInsaneFee && nFees > txMinFee * 1000000)
            return error("AcceptableInputs: : insane fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 1000000);
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 > 2 && pindexBest->nHeight+1 <= 210000)  {
        nSubsidy = 1 * COIN;
    }
    ...
    else if (pindexBest->nHeight+1 > 840001 && pindexBest->nHeight+1 <= 1000000) {
	// end game - further discussion needed
        nSubsidy = 1 * COIN;
    } else if (pindexBest->nHeight+1 > 1000000) {
	// end game - further discussion needed
        nSubsidy = 1 * COIN;
...
{
    RenameThread("Democraticoin-loadblk");
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 2) {
	ret = 0;
    } else if (nHeight >= 2 && nHeight <= 210000) {
        ret = blockValue * 25 / 35; // MN Reward 71%
    } else if (nHeight > 210000) {
	ret = blockValue / 2.04 ; // MN Reward 49%
    }
    return ret;
}
