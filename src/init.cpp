...
void Shutdown()
{
    ...
    RenameThread("Democraticoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Democraticoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Democraticoind.pid)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 35813)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDemocraticoinamount=<n> " + _("Keep N Democraticoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Democraticoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Democraticoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Democraticoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Democraticoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Democraticoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Democraticoin to complete") << "\n";
    ...
    nAnonymizeDemocraticoinAmount = GetArg("-anonymizedemocraticoinamount", 0);
    if(nAnonymizeDemocraticoinAmount > 999999) nAnonymizeDemocraticoinAmount = 999999;
    if(nAnonymizeDemocraticoinAmount < 2) nAnonymizeDemocraticoinAmount = 2;
    ...
    LogPrintf("Anonymize Democraticoin Amount %d\n", nAnonymizeDemocraticoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DVY+1000 == (.1DVY+100)*10
       10DVY+10000 == (1DVY+1000)*10
    */
